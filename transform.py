...
def convert_zip_code(zipcode: str) -> tf.float32:
    ...
    zipcode = tf.strings.to_number(zipcode)
    return zipcode
...
def get_model(show_summary: bool = False) -> tf.keras.models.Model:
    input_features = []
    for key,dim in ONE_HOT_FEATURES.items():
        input_features.append(tf.keras.layers.Input(shape=(dim+1),name=transformed_name(key)))
    ...
    embeded=hub.KerasLayer(MODULE_URL)
    reshaped_narrative=tf.reshape(input_texts[0],[-1])
    embeded_narrative=embeded(reshaped_narrative)
    
    deep_ff=tf.keras.layers.Reshape((512,),input_shape=(1,512))(embeded_narrative)
...
def  _get_serve_tf_example_fn(model,tf_transform_output):
    ...
    def serve_tf_example_fn(serialized_tf_examples):
        ...
        transformed_model=model.tft_layer(parsed_features)
        outputs=model(transformed_model)
        return {"outputs":outputs}
    return serve_tf_example_fn    
    
# tfx trainer will call the function
def run_fn(fn_args):
    ...
    tensorflow_callbacks=tf.keras.callbacks.TensorBoard(log_dir=log_dir,update_freq="batch")
